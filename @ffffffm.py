#By Akil [ TELE : @FFFFFFM - Github : AKILXXX]
...
def WinOrWhat(move):
    ...
    for _b in win : 
        for _a in move :
            if _a == _b[0] or _a == _b[1] or _a == _b[2] : _WIN.append(True) 
            if len(_WIN) == 3 : drw() ; return True
        _WIN = []
...
while (1) :
	...
	if BOX == 2 :  Player = 'X'
	clear() ; Tarn(Player) 
	if WinOrWhat(O) : print('\nO Won') ; break
	if WinOrWhat(X) : print('\nX Won') ; break
	if ( len(O) + len(X) ) == 9 : print('\nDraw') ; break
